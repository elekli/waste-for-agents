...
**MCP `list_changes(watch_id: str | None = None, since_cursor: int | None = None)`** — 語意上 `watch_id` **必填**,但**宣告為 optional 參數 + 函式內手動檢查**,不是 framework-required 硬參數。
- ⚠ **實作要點:** 若把 `watch_id` 宣告成 pydantic-required 硬參數,FastMCP 會在協定層拋 validation error,其形狀**不帶**下方要求的正規化 `cursor`、也繞過 error dict 契約(測試 #3 永不觸發)。故宣告 `watch_id: str | None = None`,省略時**由函式主動回**下面這個 dict。
...
**要求:** **四條**回傳路徑(正常空集、ownership-reject、缺 watch_id error、**auth-error**〔`server.py:297`,現行回未正規化的 `since_cursor`〕)全部經**同一個正規化 helper**——`None` 一律正規化為 `0`(或全部原樣回傳 `since_cursor`,擇一但**必須一致**),single source of truth。ownership-reject 回應須與「owned-但-空」回應**位元級相同**,含 `cursor` 欄位。(auth-error 在 watch lookup 前觸發、本身不洩漏存在性,但併入同一 helper 以免日後漂移。)
