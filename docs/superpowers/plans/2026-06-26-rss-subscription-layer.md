...
### Task 5.3: SSRF 閘補完(RSS-source PR 留的 multi-review Critical/Important)

**Files:** Modify `netguard.py`、`sources/rss.py`、`http_json.py`、`discovery.py`。`netguard.py` 已有 scheme + 內網/metadata IP 阻擋(RSS-source PR);本 task 補它 docstring 列的未完成項:
- [ ] **redirect 逐跳重驗:** fetch 改 `follow_redirects=False` 並手動逐跳 `check_outbound_url`,或停用 redirect。防 `https://attacker/redirect→169.254.169.254`。
- [ ] **出站 header allowlist:** `query.headers` 只放行 `User-Agent`/`Accept`/`If-Modified-Since`/`If-None-Match`,絕不轉 `Host`/`Authorization`/`Cookie`/`Proxy-*`。套用到 rss + http_json。
- [ ] **http_json 套上 `check_outbound_url`**(目前只有 rss/discovery 套了;http_json 仍是裸 SSRF,README 已標 MVP gap)。
- [ ] DNS-rebinding(check 解析的 IP vs httpx 實連 IP 可能不同)→ 記為 fast-follow,MVP 接受。
- [ ] 失敗測試:redirect 到內網被擋、危險 header 不轉出。
