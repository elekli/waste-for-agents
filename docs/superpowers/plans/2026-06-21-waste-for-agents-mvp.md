# waste-for-agents MVP — Agent 持久監看訂閱層(實作計畫)
...
```
                       waste-for-agents(單一常駐 process,FastAPI + uvicorn)
                ┌──────────────────────────────────────────────────────┐
...
**dogfood:** 一個 MCP(waste-for-agents)poll 另一個 MCP(Twinkle)。給 workshop 的 demo:加 waste-for-agents MCP → `create_watch` 盯某 Twinkle query → agent 下次 `list_changes` 看到 diff,全程不碰 webhook。
...
- [ ] `~/waste-for-agents` 建專案:`uv init`,`pyproject.toml` 加依賴 `mcp>=1.2`、`fastapi`、`uvicorn`,dev 依賴 `pytest`、`ruff`、`mypy`
- [ ] 套件結構:`src/waste_for_agents/{__init__.py,store.py,diff.py,sources/__init__.py,sources/base.py,sources/twinkle.py,scheduler.py,server.py}`、`tests/`
...
`src/waste_for_agents/store.py`。TDD。
...
`src/waste_for_agents/diff.py`。這是護城河,測試要厚。
...
`src/waste_for_agents/sources/`。介面留薄——elek 明示「行得通一定會抽象」。
...
`src/waste_for_agents/scheduler.py`。TDD with `FakeSource`。
...
`src/waste_for_agents/server.py`。抄 `~/npp-polhub/dd/serve/api.py` 的 FastAPI 掛載 + lifespan 模式。

### Task 5.1:FastMCP tools

- [ ] `mcp = FastMCP(name="waste-for-agents", instructions=...)`,定義:
...
- [ ] 手動驗證:`uv run python -m waste_for_agents serve` 起得來,`curl -sS -o /dev/null -w "%{http_code}" http://localhost:PORT/mcp/` 回 **406**(FastMCP streamable-http 對普通 GET 的健康訊號)→ commit
...
- [ ] `examples/sessionstart-hook.sh`:Claude Code SessionStart hook 範例,開場呼叫 waste-for-agents `list_changes` 並把變化注入 context(解掉「agent 怎麼知道要打」對短命 agent 的那一半)
- [ ] commit

### Task 6.2:README + demo walkthrough

- [ ] README 寫:一句話定位、`claude mcp add --transport http <waste-for-agents-url>/mcp/` 加法、`create_watch` 盯一個 Twinkle query 的具體範例、`list_changes` 取變化、SessionStart hook 用法
- [ ] **hosting 建議(降低 demo 風險):** 建議由 elek 常駐跑 waste-for-agents(`TWINKLE_TOKEN` 留在 server 端),只給 tester waste-for-agents 的 MCP URL——tester 不必持有 Twinkle token、不必自己跑常駐 process
- [ ] commit
...
- [ ] **end-to-end demo 驗證(真實 Twinkle):** 起 waste-for-agents → `create_watch` 盯一個會變的 Twinkle query → 等排程器跑一輪 → `list_changes` 確實取到結構化 diff、cursor 前進、無誤報。**這是 Batch 1 技術 spike 的綠燈判準**,結果寫回 HANDOFF
...
- [ ] `gh repo create waste-for-agents --private`(personal 身份),push `main`
...
- **R5 — stdio vs HTTP 常駐:** 監看要常駐 fetch,所以 waste-for-agents 必須是 HTTP 常駐 daemon(scheduler 在 lifespan 裡),不能只當 per-client spawn 的 stdio server。已採 FastAPI+uvicorn 解決。
...
2. `uv run python -m waste_for_agents serve` → `curl` `/mcp/` 回 406
3. `claude mcp add --transport http waste-for-agents http://localhost:PORT/mcp/`
